...
int MAX_LEN_ONE_SYMBOL_ENCODED = 7;

int smaz_compress(char* pin, int inlen, char* pout, int outlen)
{
    unsigned int h1, h2, h3 = 0;
    int verblen = 0, _outlen = outlen;
    char verb[256], *_out = pout;
    ...
    while (inlen)
    {
        ...
        for (; j > 0; j--)
        {
            ...
            while (slot[0])
            {
                if (slot[0] == j && memcmp(slot + 1, pin, j) == 0)
                {
                    /* Match found pin the hash table,
...
int smaz_decompress(char* pin, int inlen, char* pout, int outlen)
{
    unsigned char* c = (unsigned char*)pin;
    char* _out = pout;
    int _outlen = outlen;
